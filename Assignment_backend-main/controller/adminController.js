...
import { signInToken, tokenForVerify, handleEncryptData } from '../config/auth.js';
import { sendEmail } from '../lib/email-sender/sender.js';
...
const loginAdmin = async (req, res) => {
  try {
    console.log("Login attempt for email:", req.body.email); // ADD THIS
    console.log("Password received (before hash check):", req.body.password); // ADD THIS (Be cautious with logging raw passwords in production)

    const admin = await Admin.findOne({ email: req.body.email });

    console.log("Admin found:", admin ? true : false); // ADD THIS
    
    if (admin) {
      console.log("Admin's stored hashed password (for comparison):", admin.password); // ADD THIS
      const passwordMatches = bcrypt.compareSync(req.body.password, admin.password);
      console.log("Password comparison result:", passwordMatches); // ADD THIS
      ...
      console.log("Password did not match for email:", req.body.email); // ADD THIS
      res.status(401).send({
    ...
    console.error("Server error during login:", err); // ADD THIS
  }
};
