...
import {
  ...
  import { useContext, useState, MutableRefObject, FormEvent } from "react";
  ...
  const Coupons = () => {
    ...
    const { data, loading, error } = useAsync<Coupon[]>(CouponServices.getAllCoupons);
    // console.log('data',data)
    const [isCheckAll, setIsCheckAll] = useState<boolean>(false);
    const [isCheck, setIsCheck] = useState<string[]>([]);
    const [searchCoupon, setSearchCouponLocal] = useState("");
    ...
    const handleSelectAll = () => {
      ...
      setIsCheck(data?.map((li: Coupon) => li._id) || []);
      if (isCheckAll) {
        setIsCheck([]);
      }
    };
    ...
    const handleResetField = () => {
      setSearchCouponLocal("");
      setSearchCoupon("");
      if (couponRef && "current" in couponRef && couponRef.current) {
        (couponRef.current as HTMLInputElement).value = "";
      }
    };
  
    // Sync local searchCoupon with filter logic
    const handleSearchCouponChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      setSearchCouponLocal(e.target.value);
    ...
    return (
      <>
        ...
        />
        <BulkActionDrawer ids={allId} title="Coupons" lang={lang} data={data} />
        ...
        <AnimatedContent>
          <Card className="min-w-0 shadow-xs overflow-hidden bg-white dark:bg-gray-800 mb-5">
            <CardBody>
              <form
                onSubmit={handleSubmitCoupon as (e: FormEvent<HTMLFormElement>) => void}
              ...
              >
                <div className="flex-grow-0 md:flex-grow lg:flex-grow xl:flex-grow">
                  <Input
                    ...
                    className="w-full"
                    value={searchCoupon}
                    onChange={handleSearchCouponChange}
                    css={undefined}
