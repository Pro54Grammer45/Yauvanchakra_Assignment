...
import {
  ...
  // Types for product data (partial, adjust as needed)
  interface Product {
    _id: string;
    ...
    category: { name: string | { [lang: string]: string } };
    tag: string;
  ...
  const ProductDetails = ({ params }: { params: { id: string } }) => {
    ...
    const { data, loading } = useAsync<Product>(() => ProductServices.getProductById(id));
    ...
    useEffect(() => {
      if (!loading) {
        ...
        const varTitle = attribue?.filter((att: any) =>
              ...
              <div className="w-full flex flex-col p-5 md:p-8 text-left">
                ...
                <div className="flex flex-col mt-4">
                  ...
                  <div className="flex flex-row">
                    {data?.tag && Array.isArray(JSON.parse(data.tag)) && JSON.parse(data.tag).map((t: string, i: number) => (
                      <span
                        key={i + 1}
