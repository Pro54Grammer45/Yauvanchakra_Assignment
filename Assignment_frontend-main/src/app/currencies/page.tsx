...
import {
  ...
  const Currencies: React.FC = () => {
    ...
    const { data, loading, error } = useAsync<Currency[]>(CurrencyServices.getAllCurrency);
    ...
    const handleSelectAll = () => {
      ...
      setIsCheck(data.map((li: Currency) => li._id));
      if (isCheckAll) {
        setIsCheck([]);
      }
    };
  
    const handleSearchCurrencyChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const handleResetField = () => {
      ...
      if (currencyRef && "current" in currencyRef && currencyRef.current) {
    ...
    return (
      <>
        ...
        <BulkActionDrawer ids={allId} title="Currencies" lang={lang} data={data} />
        <MainDrawer>
          <CurrencyDrawer />
        </MainDrawer>
        <DeleteModal
        ...
        <AnimatedContent>
          <Card className="min-w-0 shadow-xs overflow-hidden bg-white dark:bg-gray-800 mb-5">
            <CardBody>
              <form
                onSubmit={handleSubmitCurrency as (e: FormEvent<HTMLFormElement>) => void}
