...
const useProductSubmit = (id: string | undefined): any => {
  ...
  const resetRefTwo = useRef<{ resetSelectedValues: () => void }>(null);
  ...
  const [isBasicComplete, setIsBasicComplete] = useState<boolean>(false);
  const [tapValue, setTapValue] = useState<string>("Basic Info");
  const [isCombination, setIsCombination] = useState<boolean>(false);
  const [attTitle, setAttTitle] = useState<any[]>([]);
  const [variantTitle, setVariantTitle] = useState<any[]>([]);
  const [attributes, setAttributes] = useState<any[]>([]);
  const [productId, setProductId] = useState<string>("");
  const [updatedId, setUpdatedId] = useState<string | undefined>(id);
  ...
  const handleRemoveEmptyKey = (obj: any): any => {
  ...
  const onSubmit = async (data: any): Promise<void> => {
    ...
    try {
      ...
      if (!defaultCategory[0]) {
        setIsSubmitting(false);
        notifyError("Default Category is required!");
        return;
      }
  ...
  const handleAddAtt = (v: any, el: any) => {
  ...
  const handleRemoveVariant = (vari: any, ext: boolean) => {
  ...
  const handleSelectInlineImage = (id: string) => {
    setImgId(id);
    setOpenModal(!openModal);
  };

  //this for variant/combination list
  const handleSkuBarcode = (value: any, name: string, id: string) => {
    variants[id][name] = value;
  };

  const handleProductTap = (e: string, value: boolean, name?: string) => {
    // console.log(e);
  ...
  const handleQuantityPrice = (value: any, name: string, id: string, variant: any) => {
    // console.log(
    //   "handleQuantityPrice",
    //   "name",
    //   name,
  ...
  const handleProductSlug = (value: string) => {
