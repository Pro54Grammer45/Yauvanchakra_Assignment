...
const useTranslationValue = (): any => {
  ...
  const cleanInvalidTranslations = (currentData: any): any => {
  ...
  const hasKeyChanged = (currentData: any, updatedData: any): boolean => {
  ...
  // translate api call
  const handleTranslateCallApi = async (text: string, tnsForm: string, tnsTo: string): Promise<any> => {
    const key =
  ...
  const handlerTextTranslateHandler = async (text: string, tnsForm: string, currentData: any): Promise<any> => {
