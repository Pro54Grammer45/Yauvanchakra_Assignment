...
const useStoreSettingSubmit = (id: string | undefined): any => {
  const { setIsUpdate } = useContext(SidebarContext);
  const [isSave, setIsSave] = useState<boolean>(true);
  const [metaImg, setMetaImg] = useState<string>("");
  const [favicon, setFavicon] = useState<string>("");
  const [enabledCOD, setEnabledCOD] = useState<boolean>(true);
  const [enabledStripe, setEnabledStripe] = useState<boolean>(true);
  const [enabledRazorPay, setEnabledRazorPay] = useState<boolean>(true);
  const [enabledFbPixel, setEnableFbPixel] = useState<boolean>(true);
  const [enabledTawkChat, setEnabledTawkChat] = useState<boolean>(false);
  const [enabledGoogleLogin, setEnabledGoogleLogin] = useState<boolean>(true);
  const [enabledGithubLogin, setEnabledGithubLogin] = useState<boolean>(false);
  const [enabledFacebookLogin, setEnabledFacebookLogin] = useState<boolean>(false);
  const [enabledGoogleAnalytics, setEnabledGoogleAnalytics] = useState<boolean>(false);
