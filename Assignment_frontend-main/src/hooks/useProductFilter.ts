...
const useProductFilter = (data: any[]): any => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newProducts] = useState<any[]>([]);
  ...
  const handleUploadProducts = (): void => {
  ...
  const handleSelectFile = async (e: any): Promise<void> => {
    e.preventDefault();
    ...
    if (file && file.type === "application/json") {
      ...
      fileReader.onload = async (event) => {
        ...
        const json = await csvToJson().fromString(text.toString());
  ...
  const handleUploadMultiple = (e: any): void => {
    if (handleDisableForDemo()) {
      return; // Exit the function if the feature is disabled
    }
  ...
  const handleRemoveSelectFile = (e: any): void => {
    // console.log('remove');
    setFileName("");
    setSelectedFile([]);
