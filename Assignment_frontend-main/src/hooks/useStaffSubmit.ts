...
const useStaffSubmit = (id: string | undefined): any => {
  ...
  const [accessedRoutes, setAccessedRoutes] = useState<any[]>([]);
  ...
  const getStaffData = async (): Promise<void> => {
    try {
  ...
  useEffect(() => {
    if (pathname === "/edit-profile" && Cookies.get("adminInfo")) {
      getStaffData();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pathname, setValue]);
